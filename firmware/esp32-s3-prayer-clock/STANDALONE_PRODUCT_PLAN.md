...
It calculates prayer times locally, displays them, plays an adhan stored in its
internal flash, and is configured through its own local web page. Docker, Home
...
- Optional FAT32 microSD card for first-time MP3 import and backup
- Wi-Fi and Bluetooth-capable radio

## Delivered Foundation

- Display, PSRAM, SD card, codec, amplifier, and speaker checks
- MP3 decoding and playback from the dedicated 8 MB internal flash partition
...
6. Trigger the internally stored adhan locally once at each enabled prayer time.
7. Expose settings, timetable, manual playback, and MP3 upload through the web UI.
8. Recover cleanly after a power cut, missing optional SD card, or lost Wi-Fi.
...
1. Power the device from USB. No microSD card is required.
...
The dashboard offers attached-speaker and Google Cast output. The ESP32
discovers receivers over mDNS, controls Google's Default Media Receiver through
a minimal local Cast V2 client, and serves the MP3 from its own byte-range HTTP
endpoint. It stores the receiver's stable ID rather than its IP address and
rediscovers it for every playback. Local playback remains the dependable
fallback whenever discovery, connection, or Cast playback confirmation fails.

## Definition of Done

The device can be powered on after a power cut, reconnect to Wi-Fi, know local
time, calculate that day's timetable, show the next prayer, and play the MP3
from internal flash at each enabled prayer time without another service.
